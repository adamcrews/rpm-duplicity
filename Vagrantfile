...
VERSION=`awk '/^%global VERSION/{print $NF}' /vagrant/spec/duplicity.spec`
RELEASE=`awk '/^%global Release/{print $NF}' /vagrant/spec/duplicity.spec`
NAME=`awk '/^Name:/{print $NF}' /vagrant/spec/duplicity.spec`
BUILD_REQUIRES=`awk -F: '/^BuildRequires/ {req_line=$2 ; split(req_line,dep_array,","); for (item in dep_array) { split(dep_array[item],final_deps," "); for (dependancy in final_deps) printf "%s ", final_deps[1]}}' /vagrant/spec/duplicity.spec`
TMPSOURCE=`awk '/^Source0:/{print $NF}' /vagrant/spec/duplicity.spec`
...
sudo yum install -y rpmdevtools ${BUILD_REQUIRES} setuptools
...
echo "Getting Source"
curl -v -L -o ~/rpmbuild/SOURCES/${NAME}-${VERSION}.tar.gz "${SOURCE}"
